import { Form, Optional, Quantitier, Content, Syntax, ZeroMore } from "./parsorama.js";
...
const Parameter = (() => {
  ...
  return class Parameter extends Syntax {
    static format = Form.new`{{{\s*${paramName}\s*${new Optional(defaultExp, Quantitier.LAZY)}(?:\s*\|\s*.*)*\s*}}}`;
...
const Template = (() => {
  ...
  class TemplateParams extends Syntax {
      ...
      params.forEach(value => {
        if (typeof value === 'string') {
          ...
          value = value.map(value => {
            if (typeof value === 'string') return new Content(value);else return value;
          });
          this.raw.push(...value);
          named.push(...value);
    ...
    *entries() {
      for (const value of this.raw) {
        if (value instanceof Content) yield [this.raw.indexOf(value), value];else if (Array.isArray(value)) yield [...value];
      }
    }

    *keys() {
      for (const value of this.raw) {
        if (value instanceof Content) yield this.raw.indexOf(value);else if (Array.isArray(value)) yield value[0];
      }
    }

    *values() {
      for (const value of this.raw) {
        if (value instanceof Content) yield value;else if (Array.isArray(value)) yield value[1];
      }
    }
  ...
  return class Template {
    static format = Form.new`{{\s*${templateName}${new ZeroMore(TemplateParams, Quantitier.LAZY)}\s*}}`;
